...
```sh
sudo apt update
sudo apt install -y build-essential cmake ninja-build git libssl-dev pkg-config 
...
cmake -B build -S . -DCMAKE_BUILD_TYPE=Release -DCMAKE_PREFIX_PATH=/opt/arrow-static -DCMAKE_EXE_LINKER_FLAGS="-s"
